      ...
      {templates.map((template, index) => (
        ...
        >
          <div className="mb-4 flex items-center justify-between gap-3">
            <span className="text-[0.72rem] font-medium uppercase tracking-[0.22em] text-[var(--muted-foreground)]">
              stage {index + 1}
            </span>
          ...
          <p className="mt-3 text-sm leading-7 text-[var(--muted-foreground)]">{template.useCase}</p>
          <ul className="mt-4 space-y-2 text-sm text-[var(--ink)]">
            {template.files.map((file) => (
              <li key={file} className="font-mono text-xs text-[var(--muted-foreground)]">
                {file}
              </li>
            ))}
          </ul>
